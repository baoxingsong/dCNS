...
def readData(which):
    path0 = "/media/bs674/1_8t/tryEmreCNNmodelDataFromKathilin/ZmTF2_category_0_gc.fasta"
    rawData=pd.read_csv(path0)
    indexes = range(rawData.shape[0])
    cnt=(np.mod(indexes,2)==0)
    X0= rawData.values[cnt]
    path1 = "/media/bs674/1_8t/tryEmreCNNmodelDataFromKathilin/ZmTF2_category_1_centered.fasta"
