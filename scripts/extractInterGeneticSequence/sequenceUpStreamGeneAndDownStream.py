...
if __name__ == '__main__':
    ...
    for chr in chromosome_gene_list1:
        ...
        similarFragments = sorted(similarFragments)
        haveChange = True
        while haveChange:
            haveChange = False
            usedList = dict()
                    ...
                    for j in range(i+1, len(similarFragments)):
                        if (similarFragments[j].ref_end - similarFragments[i].ref_start) < args.maxOutPutLength:
                            if (j not in usedList) and similarFragments[j].query_chr == similarFragments[i].query_chr:
                                if similarFragments[j].ref_start <= (similarFragments[i].ref_end+1): #they overlap with each other
                                    if(similarFragments[i].query_strand==similarFragments[j].query_strand and "+" == similarFragments[i].query_strand):
                                        if similarFragments[j].query_start >= similarFragments[i].query_start and similarFragments[j].query_start <= (similarFragments[i].query_end+1):
                                            ...
                                            usedList[j] = 1
                                            haveChange = True
                                    elif (similarFragments[i].query_strand==similarFragments[j].query_strand and "-" == similarFragments[i].query_strand): #here is difficult
                                        #
                                        if ((similarFragments[j].query_end+1) >= similarFragments[i].query_start and similarFragments[j].query_start <= similarFragments[i].query_start) or \
                                                ( similarFragments[j].query_end < similarFragments[i].query_end and similarFragments[i].query_start < similarFragments[j].query_start ):
                                            if similarFragments[j].ref_end > similarFragments[i].ref_end:
                                                similarFragments[i].ref_end = similarFragments[j].ref_end
